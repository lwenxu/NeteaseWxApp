...
Page({
  ...
  onLoad: function (options) {
    var that = this;
    wx.request({
      url: bsurl + 'getPlayListDetailById/id/'+options.pid,
      success: function (res) {
        var canplay = [];
        for (let i = 0; i < res.data.musics.length; i++) {
          // if (res.data.privileges[i].st >= 0) {
            canplay.push(res.data.musics[i])
          // }
        }
        that.setData({
          list: res.data,
          ...
          cover: id2Url.id2Url('' + (res.data.coverImgId_str || res.data.coverImgId))
        });
  ...
  playmusic: function (event) {
    ...
    music = this.data.list.musics[music];
